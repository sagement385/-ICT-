...
아래 표는 2026-07-15 최초 확인 후 2026-07-22까지 실제 연동을 재검증한 결과입니다. `실제 응답 샘플 확보 여부`는 공식 문서의 샘플/페이지 확인과 별개로, 팀이 민감정보 제거 후 테스트 fixture로 보관했는지를 구분합니다.
...
| Google Gemini generateContent (채팅 명시 사실 추출 보조) | [Gemini API 문서](https://ai.google.dev/gemini-api/docs) | 해당 없음 | Google | `x-goog-api-key` 헤더 | `GEMINI_API_KEY` | 한국어·영어 비식별 테스트 문장의 구조화 JSON 성공 응답 확인. 운영 응답 fixture 없음 | 응답 envelope·response schema·Pydantic 검증 구현. 진단·분류·추천은 구현하지 않음 | 요청 시 | 2026-07-22 확인 | 명시 상태·증상 추출을 실제 호출로 확인. 결과는 항상 사람 검토가 필요하며 실패 시 원문 수동 입력 유지 |
...
| 네이버 지도 Directions 5 | [공식 Directions 문서](https://api.ncloud-docs.com/docs/application-maps-directions5) | TODO: 상품/콘솔 서비스 ID | NAVER Cloud Platform | `x-ncp-apigw-api-key-id`, `x-ncp-apigw-api-key` 헤더 | `NAVER_MAP_CLIENT_ID`, `NAVER_MAP_CLIENT_SECRET` | 실제 후보 경로 응답 확인. 저장소 fixture는 `TEST_*` 데이터만 등록 | `summary.distance`/`summary.duration`/`path` parser, route snapshot 저장·유효 캐시 구현 | 요청 시 실시간 교통 반영 | 2026-07-22 후보 3곳 실제 경로 재검증 | 요청은 최대 10곳, 설정된 동시성 제한 안에서 부분 성공을 지원. 환자·병원 좌표는 서버 DB에서 조회 |
...
`hvidate`는 실행 PC의 timezone으로 변환하지 않습니다. `NEMC_SOURCE_TIMEZONE`이 비어 있으면 원본 문자열과 `unknown` 상태만 저장하고 정규화 시각은 `null`로 유지합니다. 제공기관 근거가 확인된 환경에서만 IANA timezone 이름을 설정하며, 잘못된 timezone은 `NEMC_SOURCE_TIMEZONE_INVALID`로 중단합니다.
