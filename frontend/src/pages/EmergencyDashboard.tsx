  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        <header className="topbar"><div><h1>충북 119 AI 응급의료 지원 시스템</h1><p>NEMC 공식 응급의료기관 후보 비교 및 이송 경로 의사결정 지원</p></div><div className="topbar-actions"><span>↻ 데이터 최신성: 원본별 확인 <b>● 제한 모드</b></span><button className="emergency-button">☎ 의사결정 보조 화면</button></div></header>
        <div className="dashboard-grid">
          ...
          <div className="center-column"><EmergencyMap patient={patient} hospitals={hospitals} recommendation={recommendation} route={route} routes={routes} /><section className="panel data-panel"><div className="section-heading"><h2>데이터 연계 현황</h2><span className="status-pill">실제 응답 기반</span></div><div className="data-status-row"><span>HIRA 병원정보</span><strong>{hospitals.length > 0 ? "응급기관과 연결" : "대기"}</strong></div><div className="data-status-row"><span>NEMC 응급의료기관 목록</span><strong>{hospitals.length > 0 ? `${hospitals.length}개 반경 후보` : "대기"}</strong></div><div className="data-status-row"><span>국립중앙의료원 실시간 상태</span><strong>hpid 연결 · 병상 의미 매핑 보류</strong></div><div className="data-status-row"><span>네이버 Directions</span><strong>{route ? `${routes.length}개 실제 경로 확인` : routingStatus ? (routingStatus.calls_limit === null ? `${routingStatus.calls_used}회 · 제한 없음` : `${routingStatus.calls_used}/${routingStatus.calls_limit}회`) : "확인 중"}</strong></div></section></div>
